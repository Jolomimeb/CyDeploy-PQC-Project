...
def receiving(conn, addr):
    try:
        while True:
            ...
            print(f"\nClient {addr} connection accepted. KEM Mode: {kem_choice}")
            ...
            #once certificate cerified, sends challenge to client and gets back its signature. Verification means that client has certificate's associated private key
            ...
            #expects ML_KEM publicKey and signature after certificate verified and challenge solved
            signed_message_payload_json = conn.recv(32768).decode()
            client_kpk, client_kpk_signature = ML_DSA_Functions.extract_signed_message_payload(signed_message_payload_json)[1:]
            ...
            if kem_choice == "H":
                ...
                signed_message_payload_json = conn.recv(32768).decode()
                #client disconnects if sent ciphertext was not verified 
                if not signed_message_payload_json:
                    print("Client could not verify ciphertext and disconnected.")
                    conn.close()
                    break
                serialized_client_epk, serialized_client_epk_signature = ML_DSA_Functions.extract_signed_message_payload(signed_message_payload_json)[1:]
            ...
            if not publicKey_info_signature_json:
                    ...
                    conn.close()
                    break
            client_encrypted_info_json, iv, signature = ML_DSA_Functions.extract_signed_AES_encrypted_message_payload(publicKey_info_signature_json)[1:]
        ...
        print(f"Client {addr} connection was forcibly closed.\n")
