...
def client_connect(client_socket_info, certificate, dpk, dsk, kem_choice):
    try:
        #client creates key pairs for both ML_KEM and X25519
...
#iterably creates a client certificate and thread, then having all clients connect simultaneously 
while True: 
    try:
        #each client is a thread
        client_threads = []

        client_num = int(input("Number of concurrent clients connecting to server between 1 and 128 inclusive: "))

        if client_num < 1 or client_num > 128:
            print("Input not in range")
            continue
        #creating each client thread with its issued certificate and KEM choice
        print("Being issued a ML_DSA certificate for each client...\n")
        for client in range(client_num):
            ...
            #generating ML_DSA keypair for each client and simulating getting issued a certificate for the public key.
            #ML_KEM and X25519 keypairs used for the actual shared secret are ephemeral (different for each communication), so it is generated in the thread itself
            ...
            client_threads.append(threading.Thread(target= client_connect, args= (client_socket_info, certificate, dpk, dsk, kem_choice))) #.start()
