...
#simulates the intermediate CA issuing a leaf certificate given the clients's name and ML-DSA public key
def issue_leaf_certificate(common_name, public_key):
    ...
    with open("Discovery_With_Certificates/intermediate_private_key.txt", "r") as file:
        intermediate_private_key = bytes.fromhex(file.read())

    #issuer is the intermediate's name
    with open("Discovery_With_Certificates/intermediate_CA.txt", "r") as file:
...
def verify_leaf_certificate(leaf_CA):
    ...
    with open("Discovery_With_Certificates/root_CA.txt", "r") as file:
